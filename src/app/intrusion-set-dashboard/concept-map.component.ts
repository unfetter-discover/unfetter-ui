...
export class ConceptMapComponent implements OnInit {
    ...
    // private conceptMap(chartElementId, infoElementId, dataJson) {
    //     let a = 800,  c = 800,  h = c,  U = 200,  K = 22,  S = 20,  s = 8,  R = 110,
    //      J = 30,  o = 15,  t = 10,  w = 1000,  F = 'elasticIn', N = '#0da4d3';
    //     let T, q, x, j, H, A, P;
    //     let L = {},  k = {};
    //     let i, y;
    //     let r = d3.tree().size([360, h / 2 - R]).separation(function(Y, X) {
    //         return (Y.parent == X.parent ? 1 : 2) / Y.depth;
    //     });
    //     // let W = d3.svg.diagonal.radial().projection(function(X) {
    //     //     return [X.y, X.x / 180 * Math.PI];
    //     // });
    //     let W = function link(d) {
    ...
    //         .on('click', O);
    //     let B = d.append('g').attr('class', 'links'),
    //         f = d.append('g').attr('class', 'ditems'),
    //         E = d.append('g').attr('class', 'nodes');
    //     let Q = d3.select('#' + infoElementId);
    ...
    //     let Z = window.location.hash.substring(1);
    //     if (Z && x[Z]) {
    //         G(x[Z]);
    //     } else {
    //         O();
    ...
    //         ad = J;
    //         Z = 180 - J;
    //         ac = (Z - ad) / (T.get('killChainPhase').length - 1);
    //         T.get('killChainPhase').forEach(function(af, ae) {
    //             af.x = ae * ac + ad;
    //             af.y = h / 2 - R;
    //             af.xOffset = S;
    //             af.depth = 1;
    ...
    //         A.entries().forEach(function(ag) {
    //             console.dir(ag.value)
    //             let ae = (ag.value[0].x - 90) * Math.PI / 180;
    //             let a2 = ((ag.value[1].x - 90) * Math.PI / 180);
    //             let bulge = 20;
    //             console.log('a2 = ' + a2)
    //             P.push({
    //                 x1: Math.cos(ae) * X + ag.value[0].xOffset,
    //                 y1: Math.sin(ae) * X,
    //                 xx: Math.cos((ae + a2) / 2) * (X + bulge) + ag.value[0].xOffset,
    //                 yy: Math.sin((ae + a2) / 2) * (X + bulge),
    //                 x2: Math.cos(a2) * X + ag.value[1].xOffset,
    //                 y2: Math.sin(a2) * X
    //             });
    //         });
    ...
    //         });
    //         D(T.get('ditems'));
    //         b(d3.merge([T.get('themes'), T.get('killChainPhase')]));
    //         C([]);
    //         m(P);
