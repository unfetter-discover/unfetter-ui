...
export class CollapsibleTreeComponent implements OnInit, OnChanges {
    ...
    public buildGraph(chartElementId, infoElementId, dataJson): void {
        ...
        function update(source) {
            ...
            nodeEnter.append('text')
                ...
                .attr('dy', '.35em')
                .attr('text-anchor', (d: any) => { return d.children || d._children  || d.type === 'intrusion-set'  ? 'end' : 'start'; })
     ...
     * @param node
     */
    private drawDiagonal(node: any): string {
        console.log(node);
        const o = {
            x: node.source.data.x0,
            y: node.source.data.y0,
            parent: {
                x: node.target.parent.x0,
                y: node.target.parent.y0
            }
        };
        // return diagonal({source: o, target: o});

        // const d: any = { x: source.x0, y: source.y0 };
        // return diagonal({source: o, target: o});
        // const d = { source: o, target: o };
        const d: any = o;
        return 'M' + d.y + ',' + d.x
