...
export class AssessmentsGroupComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.id = this.route.snapshot.params['id']
      ...
      : '';
    const routedPhase = this.route.snapshot.params['phase']
      ? this.route.snapshot.params['phase']
      : '';
    this.assessmentsDashboardService.getRiskByAttackPattern(this.id).subscribe(
      res => {
        this.riskByAttackPattern = res ? res : {};
        this.populateUnassessedPhases();
        this.activePhase = routedPhase
          ? routedPhase
          : this.riskByAttackPattern.phases[0]._id;
        this.setPhase(this.activePhase);
      },
      err => console.log(err)
    );

    this.assessmentsDashboardService.getAssessedObjects(this.id).subscribe(
      res => {
    ...
    this.assessmentsDashboardService.getById(this.id).subscribe(
      res => {
  ...
  public getRiskByPhase(phaseName) {
    const phaseObj = this.riskByAttackPattern.phases.find(
      phase => phase._id === phaseName
    );
    if (phaseObj) {
      let sum = 0,
  ...
  public setAttackPattern(attackPatternId) {
    ...
    this.assessmentsDashboardService
      ...
      .subscribe(
        res => {
          const dat: any = res;
          this.unassessedAttackPatterns = dat.filter(
            ap => !assessedAps.includes(ap.id)
          );
        },
        err => console.log(err)
      );
  ...
  public editAssessedObject(assessedObj) {
    ...
    for (let i = 0; i < assessedObj.questions.length; i++) {
      assessedObj.questions[i].selected_value.risk =
        assessedObj.questions[i].risk;
    ...
    for (
      let i = 0;
      i < this.assessment.attributes.assessment_objects.length;
      i++
    ) {
      if (
        this.assessment.attributes.assessment_objects[i].stix.id ===
        assessedObj.stix.id
      ) {
