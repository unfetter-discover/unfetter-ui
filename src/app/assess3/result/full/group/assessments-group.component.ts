...
export class AssessGroupComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public assessedObjects: Assessment3Object[];
  ...
  public initData(attackPatternIndex: number = 0): void {
    this.assessment = this.assessment || new Assessment3();
  ...
  public updateUrlIfNeeded(): void {
    ...
    if (assessmentId && phase && attackPattern) {
      this.store.dispatch(new PushUrl({ assessmentId, phase, attackPattern }));
    }
  }
   ...
   * @param {string} id
   * @return {Assessment3Question[]} questions
   */
  public getQuestions(id: string): Assessment3Question[] {
