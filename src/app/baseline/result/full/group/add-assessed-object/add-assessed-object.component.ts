...
export class AddAssessedObjectComponent implements OnInit, OnDestroy {
    ...
    public createAssessedObject(newAssessedObject, attackPattern) {
        ...
        const sub = this.baselineService
            ...
            .map((baselines) => baselines.map((el) => el.attributes))
            .subscribe(
            (assessedRes) => {
                ...
                const sub1 = this.baselineService
                ...
                this.baseline.baseline_objects.push(
                    tempAssessmentObject
                );
                const baselineToUpload: any = this.baseline;
                baselineToUpload.modified = new Date().toISOString();
                const sub2 = this.baselineService
                    .genericPatch(`${Constance.X_UNFETTER_ASSESSMENT_URL}/${this.baseline.id}`, baselineToUpload)
                    .subscribe((baselineRes) => {
