...
export class BaselineChartComponent implements OnInit {
  ...
  public calculateData(baselineGroupingFilteredCount: number, baselinesGroupingTotal: number): number {
    let data: number = 0;
    if (baselinesGroupingTotal) {
      data = Math.round((baselineGroupingFilteredCount / baselinesGroupingTotal) * 100);
    }
    return data;
  }
  ...
  public renderChart(): boolean {
    ...
    if (this.summaryCalculationService.baselinesGroupingFiltered && this.summaryCalculationService.baselinesGroupingTotal) {
      // generate uniq grouping
      const uniqueGroups = Array.from(new Set(
        Array.from(Object.keys(this.summaryCalculationService.baselinesGroupingTotal))
      ...
      uniqueGroups
        .forEach((key) => {
          this.barChartData[0].data[index] = this.calculateData(this.summaryCalculationService.baselinesGroupingFiltered[key], this.summaryCalculationService.baselinesGroupingTotal[key]);
          index = index + 1;
        });
